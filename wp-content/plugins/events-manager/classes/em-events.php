...
/**
	 * Returns events favorites
	 * @param $date
	 * @return EM_Event array()
	 */
	public static function get_events_favorites( $args = array(), $count=false ) {
		global $wpdb;
		$table_name = EM_EVENTS_TABLE;
		$sql = "
				SELECT * FROM ".$table_name."				
				WHERE wp_em_events.event_attributes like '%Evento Destacado%' 
				AND wp_em_events.event_attributes like '%Si%'; 
			";
		//$results = $wpdb->get_results(apply_filters('em_events_get_sql',$sql),ARRAY_A);
		$results = $wpdb->get_results( $sql, ARRAY_A);

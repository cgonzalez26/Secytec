 ...
 */
class EM_Events_Favorites extends WP_Widget {
	
	var $defaults;
	
    /** constructor */
    function EM_Events_Favorites() {
    	$this->defaults = array(
    		'title' => __('Events Favorites','dbem'),
    		'scope' => 'future',
    		'order' => 'ASC',
    		'limit' => 5,
    		'category' => 0,
    		'format' => '#_EVENTLINK<ul><li>#_EVENTDATES</li></ul>',
    		'nolistwrap' => false,
    	...
    	$widget_ops = array('description' => __( "Display a list of events Favorites(Destacados).", 'dbem') );
        parent::WP_Widget(false, $name = 'Events Favorites', $widget_ops);	
    }
    ...
    function widget($args, $instance) {
		...
		$events = EM_Events::get_events_favorites(apply_filters('em_widget_favorites_get_args',$instance));
		echo "<ul>";
...
add_action('widgets_init', create_function('', 'return register_widget("EM_Events_Favorites");'));
?>
...
<div id="content" class="<?php echo get_class_of_component('content', smartadapt_option( 'smartadapt_layout' )) ?>" role="main">
    ...
    $idObj = get_category_by_slug($slug); //Category de Post
 	$catid = $idObj->term_id;
	...
	//echo $parent;
	$args = array(
	     'parent' => $catid, // id of the direct parent
	     'hide_empty' => false
	);	
    ...
    foreach( $categories as $category ) {   
        ...
        if( $loop->have_posts() ) :
            ...
            <div style="width:100%; margin-bottom:30px; padding-top:10px; border-top: 1px #dfdfdf solid"><!-- daniel 14/01/15 -->
            	<a href="<?php echo get_permalink(get_page_by_slug($category->slug));?>" class="title-category"><?php echo $category->name;?></a>
            </div>
			<!-- daniel 15/12/14 -->
            <?php 
